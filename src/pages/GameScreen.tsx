  ...
  return (
    <div className="flex min-h-dvh flex-col px-4 pt-[max(1.5rem,env(safe-area-inset-top))] pb-6">
      <header className="mb-4 flex items-center justify-between">
        <button onClick={onClose} className="rounded-full bg-white px-3 py-1.5 text-sm font-semibold shadow-sm active:bg-stone-50">
          ← {t.back}
        </button>
        <button
          onClick={() => {
          ...
          className="text-sm text-stone-400 active:text-red-700"
        >
          {t.deleteGame}
        </button>
      </header>
      ...
      <div className="mb-4 grid grid-cols-2 gap-2">
        {state.standings.map((s) => {
          ...
          return (
            <div
              key={s.playerId}
              className={`rounded-2xl p-3 shadow-sm ${isWinner ? 'bg-amber-100 ring-2 ring-amber-400' : 'bg-white'}`}
            >
      ...
      {winner && (
        <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm">
          <h2 className="mb-1 text-xl font-black text-amber-600">🏆 {t.winner(winner.name)}</h2>
          <h3 className="mb-2 font-semibold text-stone-600">{t.payments}</h3>
          <ul className="space-y-1">
            {computePayments(state, game.ruleSet).map((line) => {
              const from = game.players.find((p) => p.id === line.from)!
              return (
                <li key={line.from} className="flex justify-between text-stone-700">
                  <span>{t.pays(from.name, formatEuro(line.amount), winner.name)}</span>
                </li>
              )
            })}
            <li className="mt-1 flex justify-between border-t border-stone-200 pt-2 font-bold">
          ...
          <button
            ...
            className="mt-4 w-full rounded-2xl bg-red-700 px-4 py-3 font-bold text-white active:bg-red-800"
          >
                  ...
                  <tr key={i} className="border-b border-stone-100 last:border-0">
                    ...
                    {game.players.map((p) => (
                      <td key={p.id} className="px-3 py-1.5 text-right">
                        <span className="text-stone-400">{deltas[p.id] !== 0 ? deltas[p.id] : '·'}</span>{' '}
        ...
        <div className="mt-auto space-y-2">
          {game.rounds.length > 0 && (
            <button
              ...
              className="w-full rounded-2xl py-2 text-sm font-semibold text-stone-500 active:text-stone-800"
            >
              ↩︎ {t.undoRound}
            </button>
          )}
          <button
            ...
            className="w-full rounded-2xl bg-red-700 px-4 py-4 text-lg font-bold text-white shadow-lg shadow-red-700/20 active:bg-red-800"
          >
            {t.enterRound}
          </button>
        </div>
